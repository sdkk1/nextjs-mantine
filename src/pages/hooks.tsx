...
const MantineHooks = () => {
  ...
  const { ref: refPosition, active: isActive } = useMove(setCurrentPos)
      ...
      >
        <Paper
          ...
          className={`h-40 w-40 cursor-pointer ${
            isHovered ? 'bg-indigo-500' : 'bg-orange-500'
          }`}
          ref={refHover}
        ></Paper>
        <Indicator
        ...
        >
          <Avatar
            size='lg'
            src={`${process.env.NEXT_PUBLIC_SUPABASE_STORAGE_URL}/avatars/0.6570956880851688.png`}
          />
        </Indicator>
      </Group>
      <Dialog
      ...
      <Grid my='xl'>
        <Grid.Col span={6}>
          ...
          >
            <div
              ref={refPosition}
              className='relative h-48 w-96 bg-green-700'
            >
              <BrandGithub
